...
Step 3: VERIFICATION GATE (per commit)
  → Run the checks that actually exist: `npm run typecheck` + `npm run build` (this repo has no lint script and no test harness)
  → Run concrete manual shell scenarios for the behavior changed
...
Step 4: SPEC COMPLIANCE (only when SDD was explicitly used)
  ...
  → Verify the implementation matches the ODD task doc's acceptance criteria
...
## Quality Standards
- Run `npm run typecheck` + `npm run build` before considering work done (no lint script)
- Follow existing code conventions (check neighboring files)
- Each commit = one concern, descriptive messages
- Add tests when a harness exists; this repo has none, so verify with manual scenarios instead
