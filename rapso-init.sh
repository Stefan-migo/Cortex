...
RAPSO_PACK_DIR="${RAPSO_PACK_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"
...
if [ "$INSTALL_GLOBAL" = true ]; then
  ...
  if [ -f "$RAPSO_PACK_DIR/commands/rapso-init.md" ]; then
    cp "$RAPSO_PACK_DIR/commands/rapso-init.md" "$GLOBAL_CMD_DIR/rapso-init.md"
    echo "✅ /rapso-init command instalado globalmente en $GLOBAL_CMD_DIR"
    echo ""
  fi
fi
...
if [ "${RAPSO_WORKTREE_PROVISION:-false}" = true ]; then
...
echo "$EXISTING" > "$PROJECT_DIR/.opencode/_rapso_mcp_tmp.json"
python3 -c "
import json, os

src = '$PROJECT_DIR/.opencode/_rapso_mcp_tmp.json'
with open(src) as f:
    data = json.load(f)

project_dir = '$PROJECT_DIR'
rapso_dir = '$RAPSO_PACK_DIR'
...
plugin_path = os.path.join(rapso_dir, '.opencode/plugins/graphify.js')
...
"
mv "$PROJECT_DIR/.opencode/_rapso_mcp_tmp.json" "$MCP_FILE"
...
export RAPSO_SRC="$RAPSO_PACK_DIR"
...
rapso_src = os.environ['RAPSO_SRC']
...
with open(reg_path, 'w') as f:
    f.write('# Skill Registry — rapso-init\n\n')
    f.write(f'Source: Rapsodia pack at {rapso_src}\n')
    ...
    f.write('\n')
    f.write('<!-- generated by rapso-init.sh -->\n')
...
# ─── 7. Install /rapso-init command in project ────────────────────────────
echo "🔗 Instalando comando /rapso-init..."

mkdir -p "$PROJECT_DIR/.opencode/commands"
CMD_SRC="$RAPSO_PACK_DIR/commands/rapso-init.md"
CMD_TARGET="$PROJECT_DIR/.opencode/commands/rapso-init.md"
if [ -f "$CMD_SRC" ]; then
  if [ ! -f "$CMD_TARGET" ]; then
    cp "$CMD_SRC" "$CMD_TARGET"
    echo "  ✅ /rapso-init command instalado"
  else
    echo "  ✅ /rapso-init command ya existe"
  fi
fi
